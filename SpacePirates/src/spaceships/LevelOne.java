...
/**
 * LevelOne class that sets up characteristics for level one of spaceship
 */

public class LevelOne extends Spaceship {
	Scanner in = new Scanner(System.in);

	/**
	 * constructor to initialize all characteristics for level one spaceship
	 * sets one of each equipments
	 * 
	 * @param members
	 *            - list of members
	 * @param food
	 *            - list of foods
	 */
	public LevelOne(ArrayList<Creature> members, ArrayList<Food> food) {
		super("Level One", 1, 100, members, food, 25, 100);
		partsInUse = new Equipment[7]; // [cannon, barrier, hyperdrive, pfc, fp]
		partsInUse[0] = new Cannon(1);
		partsInUse[1] = new Barrier(1);
		partsInUse[2] = new Hyperdrive(1);
		partsInUse[3] = new PowerFluxCapacitator(1);
		partsInUse[4] = new FulstarPlate(1);
		partsInUse[5] = new Laser(1);
	...
	public boolean canUpgrade() {
		if (!isBroken()) {
			...
			if (cannonCounter >= 1 && barrierCounter >= 1 && fpCounter >= 1 && hdCounter >= 1 && laserCounter >= 1
					&& pfcCounter >= 1) {
				return true;
			}
			return false;
